// Firebase Configuration for Football Play Maker
// Using the same Firebase project as TV Time Manager and Family Connect
...
function initializeFirebaseIfReady() {
    if (typeof firebase !== 'undefined' && window.location.protocol !== 'file:') {
        try {
            console.log('Initializing Firebase for Football Play Maker...');
            firebase.initializeApp(firebaseConfig);
            db = firebase.firestore();
            // Expose db on window for React app to access
            window.db = db;
